...
def reduction_tree(H,G,Pi=np.zeros((2,2)), rho=1000, method='LP', lambda_IBP = 1, npool=1):
    ...
    * time_tot: total time in second to compute the reduction
    ...
    for t in range(T-1,-1,-1):
        ...
        # Parrallel work using POOL:
        _loop_subtrees_partial = partial(_loop_subtree_n, t, G, H, nodesG, nodesH, list_m, list_n, D_ij, Pi,
                                         method, lambda_IBP, rho, epsilon)
        with Pool(npool) as pool:
            outputs = pool.map(_loop_subtrees_partial, [n for n in list_n])
        ...
        for i_n, n in enumerate(list_n):
            # outputs = {}
            # outputs[i_n] = _loop_subtree_n(t, G, H, nodesG, nodesH, list_m, list_n, D_ij, Pi, method,
            #                           lambda_IBP, rho, epsilon, n)
            Pi_k[n] = outputs[i_n]["Pi_k"]
            D_ij_t.append(outputs[i_n]["D_ij_t"])
            ancestor_n.extend(outputs[i_n]["ancestor_n"])
            if i_n == 0:
                ancestor_m = outputs[i_n]["ancestor_m"]
...
def _loop_subtree_n(t, G, H, nodesG, nodesH, list_m, list_n, D_ij, Pi, method, lambda_IBP, rho, epsilon, n):
    ancestor_m = []
    ancestor_n = []
